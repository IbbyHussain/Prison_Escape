...
EBTNodeResult::Type UCTask_IncrementPatrolPathIndex::ExecuteTask(UBehaviorTreeComponent& Owner, uint8* node_memory)
{
	...
	int const NumberOfPoints = AISamuraiReference->GetPatrolPathAI()->num();
	...
	Controller->get_blackboard()->SetValueAsInt(bb_Keys::PatrolPointIndex, 
		(Direction == EDirectionType::Forward ? std::abs(++index) : std::abs(--index)) % NumberOfPoints);
